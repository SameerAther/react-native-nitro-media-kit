...
- [`android/`](android): All your `android`-specific implementations.
  - [`build.gradle`](android/build.gradle): The gradle build file. This contains four important pieces:
    ...
    3. Adds all Nitrogen files (`apply from: '.../MediaKit+autolinking.gradle'`)
  ...
  - [`CMakeLists.txt`](android/CMakeLists.txt): The CMake build file to build C++ code. This contains four important pieces:
    1. Creates a library called `MediaKit` (same as in `nitro.json`)
    2. Adds all Nitrogen files (`include(.../MediaKit+autolinking.cmake)`)
  ...
  - [`src/main/java/com/margelo/nitro/mediakit/`](android/src/main/java/com/margelo/nitro/mediakit/): All Kotlin implementations.
    - [`MediaKitPackage.java`](android/src/main/java/com/margelo/nitro/mediakit/MediaKitPackage.java): The react-native package. You need this because the react-native CLI only adds libraries if they have a `*Package.java` file. In here, you can autolink all Kotlin HybridObjects.
...
- [`MediaKit.podspec`](MediaKit.podspec): The iOS podspec build file to build the iOS code. This contains three important pieces:
